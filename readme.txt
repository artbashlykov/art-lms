...
= 2.17.42 =
* Admin orders list: add Product column (payment button title).
* Admin orders list: toggle columns in Screen Options.
* Admin orders list: use icons for View / Edit / Delete actions.

= 2.17.41 =
* Admin order edit: match Amount and Status field width to Name / Phone / Payment button.

= 2.17.40 =
* Admin order view: match Amount and Status field width to Name / Phone / Email.

= 2.17.39 =
* Payments: defer purchase/admin emails after paid webhook so gateways (e.g. Prodamus) get a fast HTTP 200; mark_paid and access grant stay synchronous.

= 2.17.38 =
* Access: daily cron marks expired material access as expired.
* Access: keep access when another non-expired grant for the same material still exists.

= 2.17.37 =
* Frontend: hide the WordPress admin bar for all users except administrators.

= 2.17.36 =
* Admin orders: filter by access expiry status and expiration date range.
* Payment buttons: fix admin search so it only matches buttons (title/product name), not unrelated posts.

= 2.17.35 =
* Admin orders: add “Access expires” column (dash, date, or red EXPIRED).
* Payment buttons and checkout: format prices with thousands separators (e.g. 5 000 ₽).

= 2.17.34 =
* Admin orders: payment button dropdown and order views show the button title (not the product name).

= 2.17.33 =
* Prodamus: always acknowledge missing orders with HTTP 200 (stops merchant error emails from direct/Tilda links); LMS paid flow unchanged when the order is found.

= 2.17.32 =
* Prodamus: ignore foreign payment notifications (no ART LMS `ord_…` label) with HTTP 200 so Prodamus does not email errors; real LMS checkouts still mark paid and grant access as before.

= 2.17.31 =
* Emails: open all HTML links in a new tab (target=_blank).
* Purchase email: blank line between “Личный кабинет” and login/password block.
